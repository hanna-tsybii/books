...
function viewBooksCategories() {
	fetch('https://6514585f8e505cebc2eb27cf.mockapi.io/categories', {
		method: 'GET',
...
function viewCatalogBooks(page = 1) {
	fetch(`https://6514585f8e505cebc2eb27cf.mockapi.io/books/?page=${page}&limit=8`, {
		method: 'GET',
		headers: { 'content-type': 'application/json' },
	}).then(res => {
		if (res.ok) {
...
function selectProductsByCategory(event) {
	...
	if (categoryRemember == category) {
		...
		fetch(`https://6514585f8e505cebc2eb27cf.mockapi.io/books/?categoryid=${category}`, {
			method: 'GET',
			headers: { 'content-type': 'application/json' },
		}).then(res => {
			if (res.ok) {
...
function displayNumFavoriteBooks() {
	...
	const iconSaved = document.querySelector('.menu__saved .icon');
	...
	if (savedBooks.length == 0) {
		iconSaved.classList.remove('saved');
		spanSavedNum.style.display = 'none';
	} else {
		iconSaved.classList.add('saved');
...
function displaySavedBooks() {
	...
	if (document.querySelector('#book-saved-list') != null) {
		...
		if (savedBooks.length == 0) {
			...
			fetch(`https://6514585f8e505cebc2eb27cf.mockapi.io/books/`, {
				method: 'GET',
				headers: { 'content-type': 'application/json' },
			}).then(res => {
				if (res.ok) {
...
function search() {
	...
	if (searchValue.length == 0) {
		...
		fetch(`https://6514585f8e505cebc2eb27cf.mockapi.io/books/?search=${searchValue}`, {
			method: 'GET',
			headers: { 'content-type': 'application/json' },
		}).then(res => {
			if (res.ok) {
